...
class AntColonyAlgorithm :
    ...
    def in_a_generations(self, iteration ):
        ...
        3. find out the best solution in current generation
            ...
            2. find out minimum length and compare it with historical one and get the best solution
        4. update pheromone table
            1. volatilization index and newly accumulated
        :return:
        """
        def shuffle_cities( n, m):
            cities = np.zeros( m )
            for i in range( m ):
                cities[i] = random.randint( 0, n-1 )
            return cities

        def choose_next_city( forbiddenCities, n, A, pheromoneTable, heuristicTable, alpha, beta  ):
        ...
        def current_best_solution(self, iteration ):

            def route_length( self ):
                length = np.zeros( self.m )
                for i in range( self.m ):
                    for j in range( self.n - 1 ):
                        length[i] = length[i] + self.D[ int(self.Table[i, j] ), int( self.Table[i, j + 1 ] )]
                    length[i] = length[i] + self.D[ int( self.Table[i, self.n - 1 ] ), int( self.Table[i, 0] ) ]
                return length

            length = route_length( self )
        ...
        def update_pheromone_table( self, length ):
        ...
        self.Table[:, 0] = shuffle_cities( self.n, self.m )
        ...
        for i in range( self.m ):
            for j in range( 1, self.n ):
                self.Table[i, j ] = choose_next_city( self.Table[i, 0:j ], self.n, self.Table[i, j-1], self.Tau,
                                                    self.Heu_F, self.alpha, self.beta )
                # print 'iteration %d, ant %d, fist city %d, next city %d' % ( iteration, i,
                #                                                              self.Table[i, 0], self.Table[i, j ])
        length = current_best_solution(self, iteration )
        update_pheromone_table( self, length )
